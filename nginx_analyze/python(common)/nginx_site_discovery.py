...
for file in file_list:
        access_log = conf_parse(nginx_etc_path, file, 'access_log').procedure()
        server_name = conf_parse(nginx_etc_path, file, 'server_name').procedure()
        listen_port = conf_parse(nginx_etc_path, file, 'listen').procedure()


        dic_content = {
        "{#ACCESS_LOG}":  os.path.join(nginx_base, access_log),
